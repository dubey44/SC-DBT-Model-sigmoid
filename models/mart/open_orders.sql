...
otm_order.shipment_no as shipment_no,
split_part(otm_shipment.servprov_gid,'.',2) as scac,
-- sales_order_header.order_type as order_type,
split_part(otm_shipment.attributes,'.',2) as otm_order_type,
split_part(otm_shipment.shipment_type_gid,'.',2) as shipment_type,
otm_order.order_no as otm_order_release_type_gid,
...
otm_shipment.transport_mode_gid as transport_mode_gid,
...
otm_shipment.shipment_refnum_value as bol,
...
otm_shipment.total_item_package_count as total_item_package_count,
...
otm_shipment.PICKUP_ETA at TIME ZONE ORIGIN_TIMEZONE as pickup_eta,
-- CONVERT_TIMEZONE(ORIGIN_TIMEZONE,PU.PICKUP_PTA) as pickup_pta,
otm_shipment.PICKUP_PTA at TIME ZONE ORIGIN_TIMEZONE as pickup_pta,
-- CONVERT_TIMEZONE(ORIGIN_TIMEZONE,PU.APPOINTMENT_PICKUP) as site_pickup_appt,
otm_shipment.site_pickup_appt at TIME ZONE ORIGIN_TIMEZONE as site_pickup_appt,
...
otm_shipment.DELIVERY_ETA at TIME ZONE otm_shipment.dest_timezone_gid AS delivery_eta,
-- CONVERT_TIMEZONE(DEST_TIMEZONE,DELIVERY_PTA) as delivery_pta,
otm_shipment.DELIVERY_PTA at TIME ZONE otm_shipment.dest_timezone_gid as delivery_pta,
-- CONVERT_TIMEZONE(DEST_TIMEZONE,site_delivery_appt) as site_delivery_appt,
otm_shipment.site_delivery_appt at TIME ZONE otm_shipment.dest_timezone_gid as site_delivery_appt,
...
otm_shipment.ACTUAL_ARRIVAL_AT_SHIPPER at TIME ZONE ORIGIN_TIMEZONE as actual_arrival_at_shipper,
-- CONVERT_TIMEZONE(ORIGIN_TIMEZONE,ACTUAL_DEPARTURE_FROM_SHIPPER) as actual_departure_from_shipper
otm_shipment.actual_departure_at_shipper at TIME ZONE ORIGIN_TIMEZONE as actual_departure_from_shipper,
-- CONVERT_TIMEZONE(DEST_TIMEZONE,ACTUAL_ARRIVAL_AT_CONSIGNEE) as actual_arrival_at_consignee,
otm_shipment.ACTUAL_ARRIVAL_AT_CONSIGNEE at TIME ZONE DEST_TIMEZONE as actual_arrival_at_consignee,
-- CONVERT_TIMEZONE(DEST_TIMEZONE,ACTUAL_DEPARTURE_FROM_CONSIGNEE) as actual_departure_from_consignee,
otm_shipment.ACTUAL_DEPARTURE_AT_CONSIGNEE at TIME ZONE DEST_TIMEZONE as actual_departure_from_consignee,
...
otm_shipment.rate_geo_gid as rate_geo_gid,
otm_shipment.rate_offering_gid as rate_offering_gid,
-- otm_shipment.shipment_release_status as shipment_release_status,
otm_shipment.num_order_releases as num_order_releases,
otm_shipment.last_event_group_gid as last_shipment_status,
d1.status_code_gid as delivered_indicator,
otm_shipment.domain_name as domain_name,
otm_order.total_shipment_unit_count as total_shipment_unit_count,
otm_order.total_weight as total_weight,
otm_order.total_weight_uom_code as total_weight_uom_code,
otm_order.total_volume as total_volume,
otm_order.total_volume_uom_code as total_volume_uom_code,
otm_shipment.weight_utilization as weight_utilisation,
otm_shipment.volume_utilization as volume_utilisation,
otm_order.priority as priority,
otm_order.mode_profile_gid as mode_profile_gid,
...
left join otm_order on sales_order_header.customer_purchase_order_number = otm_order.order_no
left join otm_shipment on otm_shipment.shipment_id = otm_order.shipment_no
left join otm_shipment_status x1 on x1.shipment_gid = otm_shipment.shipment_id
left join otm_shipment_status d1 on d1.shipment_gid = otm_shipment.shipment_id
left join otm_shipment_status xp on xp.shipment_gid = otm_shipment.shipment_id
left join otm_shipment_status xd on xd.shipment_gid = otm_shipment.shipment_id
left join otm_shipment_status l1 on l1.shipment_gid = otm_shipment.shipment_id
left join gross_value_cte on gross_value_cte.vbeln = otm_shipment.shipment_id
left join origin_cte on origin_cte.vbeln = otm_shipment.shipment_id
--  (shipment_no)
